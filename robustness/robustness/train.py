...
def eval_model(args, model, model_fp, loader, store):
    """
...
#                                         model, None, 0, False, writer)
    ...
    if args.adv_eval: 
        ...
        if args.custom_adv_loss is not None:
            adv_prec1, adv_loss = _model_loop_diva(args, 'val', loader, 
                                        model, None, 0, True, writer,model_fp)
        else:
...
def _model_loop_diva(args, loop_type, loader, model, opt, epoch, adv, writer, orig_model):
    """
    *Internal function* (refer to the train_model and eval_model functions for
    how to train and evaluate models).
    ...
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        #######use the ones are agreed by the two models at first
        norm = helpers.InputNormalize(ch.tensor([0.485, 0.456, 0.406]), ch.tensor([0.229, 0.224, 0.225]))
        o_q = model(inp,with_image=False)
        o_fp = orig_model(norm(inp).cuda(non_blocking=True))
        q_logits = o_q[0] if (type(o_q) is tuple) else o_q
        fp_logits = o_fp[0] if (type(o_fp) is tuple) else o_fp
        _, q_p = q_logits.topk(1, 1, True, True)
        _, fp_p = fp_logits.topk(1, 1, True, True)
        q_p = q_p.t()[0]
        fp_p = fp_p.t()[0]
        index = ((q_p + fp_p - target*2) == ch.zeros(target.shape,).cuda(non_blocking=True))
        inp = inp[index]
        target = target[index]
        if target.size(0) == 0:
            continue
        ...
        output, final_inp = model(inp, target=target, make_adv=adv,
                                  **attack_kwargs)
        output_orig = orig_model(final_inp)
        ...
        model_logits = output[0] if (type(output) is tuple) else output
        model_logits_orig = output_orig[0] if (type(output_orig) is tuple) else output_orig
        
        _, q_p = model_logits.topk(1, 1, True, True)
        _, fp_p = model_logits_orig.topk(1, 1, True, True)
        q_p = q_p.t()[0]
        fp_p = fp_p.t()[0]
        index_fp = (fp_p == target)
        index_q = (q_p != target)
        
        print( 'num_agreed: '+ str(target.size(0)) + '   num_q_incorrect: ' + str(len(q_p[index_q])) + '   num_fp_correct: '+ str(len(q_p[index_fp])) +'    num_sucess: ' + str(q_p[ch.logical_and(index_fp ,index_q)].size(0)))
        continue
        ...
        try:
            maxk = min(5, model_logits.shape[-1])
            if has_attr(args, "custom_accuracy"):
                prec1, prec5 = helpers.accuracy_diva(model_logits, model_logits_orig,target, topk=(1, maxk))
                prec1, prec5 = prec1[0], prec5[0]
            else:
