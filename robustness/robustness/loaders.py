...
class RandomSampler(ch.utils.data.Sampler):
...
def make_loaders(workers, batch_size, transforms, data_path, data_aug=True,
                ...
                only_val=False, shuffle_train=False, shuffle_val=False, seed=1,
    ...
    **INTERNAL FUNCTION**
    ...
    train_dataset_split = Subset(test_set,indices[:int(len(test_set)*0.4)])
    val_dataset_split = Subset(test_set,indices[int(len(test_set)*0.4):])
    train_loader = DataLoader(train_dataset_split, batch_size=val_batch_size, 
            shuffle=shuffle_train, num_workers=workers, pin_memory=True)
    sampler = RandomSampler(val_dataset_split)
    sampler.set_seed(310) # fix the seed so the shuffle will be the same everytime
    test_loader = DataLoader(val_dataset_split, batch_size=val_batch_size, 
            shuffle=False, num_workers=workers, pin_memory=True, sampler = sampler)
    print(len(test_set))
