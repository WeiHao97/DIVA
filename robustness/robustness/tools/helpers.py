...
def accuracy_diva(output,output_orig, target, topk=(1,), exact=False):
    """
    ...
    with ch.no_grad():
        ...
        if len(target.shape) > 1:
            ...
            assert output_orig.shape == target.shape, \
                "Detected binary classification but output shape != target shape"
            return [ch.round(ch.sigmoid(output)).eq(ch.round(target)).float().mean()], [-1.0]
        ...
        _, pred_orig = output_orig.topk(maxk, 1, True, True)
        pred_orig = pred_orig.t()
        correct_orig = pred_orig.eq(target.view(1, -1).expand_as(pred_orig))

        res = []
        res_exact = []
        for k in topk:
            if k==1:
                correct_k = (~correct[:k].reshape(-1).bool()).int()
                correct_k_orig = correct_orig[:k].reshape(-1).int()
                correct_1 = correct_k + correct_k_orig
                ck_sum = ch.tensor([list((correct_1==2).nonzero().reshape(-1).size())[0]]).float().cuda()
            else:
