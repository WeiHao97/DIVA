...
#Famous people subset to attack
...
#Targetted face recognition attack
def attack():
    
    data=[] #Data will be stored here

    for i,celeb_image in enumerate(test_x):
        #Only pick our subset of celebs to attack
        if decode[str(test_y[i])] not in famous:
            continue
            
        winners = {} #People we can attack
        image = celeb_image
        print("Real Label + " + decode[str(test_y[i])])
        orig_label = test_y[i]
        is_bad_image = False
        
        # Go through all other possible celebs and try to make an attack
        for celeb in range(150):    
            ...
            #If both the quantized and fp models disagree on a label we skip the image
            ...
            for iters in range(grad_iterations):
                ...
                #If we successfully fool the quantized model but not the fp model we count it as a success
                if label2 == celeb:
