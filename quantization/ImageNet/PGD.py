...
# Construct models
if mode == 'm':
    model_ = tf.keras.applications.MobileNet(input_shape= (img_rows, img_cols,num_channel))
    ...
    model_ = ResNet50(input_shape= (img_rows, img_cols,num_channel))
    q_model = tfmot.quantization.keras.quantize_model(model_)
    ...
    model_ = tf.keras.applications.DenseNet121(input_shape=(img_rows, img_cols,num_channel))
    # Create a base model
    base_model = model_
...
calc_normal_success(second,secondk,mydataset,
                   folderName=net + 'net_imagenet_images_second', filterName=net +'net_imagenet_filters_second',dataName='second', dataFolder=net +'net_imagenet_data_second', locald ='./results/PGD/' + net + 'net/' )