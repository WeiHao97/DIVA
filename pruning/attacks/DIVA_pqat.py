# Usage: python3 DIVA_pqat.py [r/d/m](stands for resnet/densenet/mobilenet) [n](gpu index)
import numpy as np
import os
import sys
...
if mode == 'm':
    ...
    p_model.load_weights("../../weights/pqat_model_40_resnet50.h5")
    model.trainable = False
    p_model.trainable = False
    ...
    with tfmot.quantization.keras.quantize_scope({'DefaultBNQuantizeConfig': DefaultBNQuantizeConfig, 'NoOpQuantizeConfig': NoOpQuantizeConfig}):
        p_model = tfmot.quantization.keras.quantize_apply(annotated_model)

    model = tf.keras.applications.DenseNet121(input_tensor = p_model.input)
    model.load_weights("../../weights/fp_model_40_densenet121.h5")
    p_model.load_weights("../../weights/pqat_model_40_densenet121.h5")
    model.trainable = False
    p_model.trainable = False
...
def second(image,label):
    ...
    for iters in range(0,grad_iterations):
        with tf.GradientTape() as g:
            ...
            loss2 = K.mean(p_model(preprocess(input_image+A)[None,...], training = False)[..., orig_label])
...
def calc_normal_success(method, methodk, ds, folderName='', filterName='',dataName='',dataFolder='',locald = ''):
    ...
    print(locald, os.path.exists(locald))
...
calc_normal_success(second,secondk,mydataset,
                   folderName=net + 'net_imagenet_images_second', filterName=net +'net_imagenet_filters_second',dataName='second', dataFolder=net +'net_imagenet_data_second', locald ='../results/pqat/DIVA/'+net+'net/' )
